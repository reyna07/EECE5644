...
# DeepLift in explainer_improved function
def explainer_improved(model, labels, DEVICE, img, true_label):
    ...
    # Compute the attribution scores using Deeplift for true label

    attribution = DeepLift(model)
                       
    attr_true = attribution.attribute(inputs=input, target=true_label)

    # Compute the attribution scores using Deeplift for predicted label
    attr_pred = attribution.attribute(inputs=input, target=predicted_label)

    # Transform the image to original scale
    input *= torch.tensor([0.229, 0.224, 0.225]).view(1, 3, 1, 1) + torch.tensor([0.485, 0.456, 0.406]).view(1, 3, 1, 1)
    ...
    ax[2].set_title(f"Predicted: {labels[predicted_label][0]}", fontsize=48)
    ax[0].set_xticks([])
    ax[0].set_yticks([])
    ax[1].set_xticks([])
    ax[1].set_yticks([])
    ...
    FIG_PATH = "hw4\DeepLift" + str(true_label) + ".png"
...
if __name__ == "__main__":
    ...
    #true_label = 1  # This is the label number for the predicted label
    ...
    # Call the explainer_improved function
