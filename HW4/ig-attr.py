...
# IG in explainer_improved function
def explainer_improved(model, labels, DEVICE, img, true_label):
    ...
    # Compute the attribution scores using IG for true label

    attribution = IntegratedGradients(model)    
    
    attr_true = attribution.attribute(inputs=input, target=true_label)

    # Compute the attribution scores using IG for predicted label
    ...
    FIG_PATH = "hw4\IntegratedGradient" + str(true_label) + ".png"
